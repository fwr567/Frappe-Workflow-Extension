...
class NLWorkflowAction(Document):
    pass


def on_doctype_update():
    frappe.db.add_index(
        "NL Workflow Action", ["reference_name", "reference_doctype", "status"]
    )
...
def get_permission_query_conditions(user):
    ...
    WorkflowAction = DocType("NL Workflow Action")
    WorkflowActionPermitted = DocType("NL Workflow Action Permitted Role")
...
def process_workflow_actions(doc, state):
    ...
    clear_doctype_notifications("NL Workflow Action")
    ...
    roles = {(t.approver_type, t.allowed) for t in next_possible_transitions}
...
def clear_old_workflow_actions_using_user(doc, user=None):
    ...
    if frappe.db.has_column("NL Workflow Action", "user"):
        frappe.db.delete(
            "NL Workflow Action",
            {
...
def has_workflow(doctype: str):
    """Return the workflow name if a NL Workflow exists for the given doctype."""
    workflow = frappe.db.get_value("NL Workflow", {"document_type": doctype})
...
def get_users_next_action_data(transitions, doc):
    ...
    for transition in transitions:
        ...
        filtered_users = [
            user
            for user in users
            if has_approval_access(user, doc, transition) and user_has_permission(user)
        ]
        if doc.get("owner") in filtered_users and not transition.get(
...
def create_workflow_actions_for_roles(roles, doc):
    ...
    for approver_type, approver in roles:
...
def send_workflow_action_email(doc, transitions):
    ...
    for data in users_data.values():
        email_args = {
            ...
            "args": {
                "actions": list(deduplicate_actions(data.get("possible_actions"))),
                "message": message,
            },
...
def get_common_email_args(doc):
    ...
    if email_template:
        ...
        subject = _("NL Workflow Action") + f" on {doctype}: {docname}"
